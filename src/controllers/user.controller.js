...
// for update files we need multer and middlewares
...
const updateUserAvatar = asyncHandler(async (req, res) => {
  ...
  return res
    .status(200)
    .json(new ApiResponse(200, user, "Avatar is Added Successfully "));
});
...
const updateUserCoverImage = asyncHandler(async (req, res) => {
  ...
  return res
    .status(200)
    .json(new ApiResponse(200, user, "coverImage is Added Successfully "));
});

const getUserChannelProfile = asyncHandler(async (req, res) => {
  const { username } = req.params;
  if (!username?.trim()) {
    throw new ApiError(400, "usrename not found");
  }
  ...
  if (!channel?.length) {
    throw new ApiError(404, "Channel is not created");
  }
      ...
      $lookup: {
        ...
        pipeline: [
          ...
          },
          {
            $addFields: {
              owner: {
                // array elements at or first to get first element to pick from field we use $
                $first: "$owner",
              },
            },
          },
